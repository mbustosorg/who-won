...
object WhoWonTables {
  ...
  case class PlayerWinnings(userName: String, winnings: List[Double], percentage: List[Int], roi: List[Double])
...
trait WhoWonJsonProtocol extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  implicit val playerWinnings = jsonFormat4(PlayerWinnings)
