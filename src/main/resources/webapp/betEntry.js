...
$(document).ready(function() {
    ...
    function submitBet() {
        ...
        var dataString = '{\"userName\": \"' + userName + '\", \"bookId\": ' + bookId+ ', \"year\": ' + loadingTimestamp.getFullYear() + ', \"spread\": ' + spreadAmount + ', \"amount\": ' + betAmount + '}';
        $.ajax({
            type: "POST",
            url: '/bets/',
            dataType: "json",
            ...
            statusUpdate(results, bookId);
        }).error(function(results) {
            errorUpdate(results, bookId);
        });
		$.ajax({
    ...
    function errorUpdate(results, bookId) {
         ...
         } else if (results == 'Unknown BookId') {
            $('#submitResult').addClass('alert-danger');
            $('#submitResult').removeClass('alert-success');
